...
@interface TableActivitiesViewController (){
...
@implementation TableActivitiesViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ( indexPath.row < activities.count){
       ...
       NSString *DateIfinal = [formatter stringFromDate:[[activities objectAtIndex:indexPath.row]finalData]];
        ...
        NSLog(@"FINAL DATE: %@", finalDate.text);
        descr.text = [[activities objectAtIndex:indexPath.row]activityDescription];

        
         return cell;
    }else{
        UITableViewCell *lastCell = [tableView dequeueReusableCellWithIdentifier:@"lastCell" forIndexPath:indexPath];

        UIButton *add = (UIButton *)[lastCell viewWithTag:8];
        return lastCell;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        NSString *inStr = [NSString stringWithFormat: @"%ld", self.idTravel];
        

        [manager removeActivity:[activities objectAtIndex:indexPath.row]andUser:usr andId:inStr];
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row < activities.count) {
    aux = indexPath;
    [self performSegueWithIdentifier:@"toViewActivityViewController" sender:[activities objectAtIndex:indexPath.row]];
    }
}
...
-(IBAction)addActivityButtonTouched{
    NSLog(@"holaaa");
    [self performSegueWithIdentifier:@"toAddActivityViewController" sender:nil] ;
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.identifier isEqualToString:@"toAddActivityViewController"]){
        ...
        if([segue.identifier isEqualToString:@"toViewActivityViewController"]){
